 ...
 */
function boom_form_install_configure_form_alter(&$form, $form_state) {
 ...
 */
function boom_install_tasks() {
  $tasks = array();

  $tasks['boom_setup_variables'] = array(
    'display_name' => st('Set Variables'),
    'display' => FALSE,
  );

  $tasks['boom_setup_og_permissions'] = array(
    'display_name' => st('Set Blocks'),
    'display' => FALSE,
  );

  // Run this as the last task!
  $tasks['boom_setup_rebuild_permissions'] = array(
 ...
 */
function boom_setup_variables() {
  $variables = array(
    // Features default export path.
    'features_default_export_path' => 'profiles/boom/modules/custom',
    // Mime-mail.
    'mimemail_format' => 'full_html',
    'mimemail_sitestyle' => FALSE,
    'mimemail_name' => 'Boom',
    'mimemail_mail' => 'info@boom.com',
    // jQuery versions.
 ...
 * created.
 */
function boom_setup_og_permissions() {
 ...
 * installation.
 */
function boom_setup_rebuild_permissions() {
  node_access_rebuild();
}
